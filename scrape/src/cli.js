...
const INPUT_FILE = path.join(SCRAPER_DIR, 'digimon.json');
const OUTPUT_FILE = path.join(SCRAPER_DIR, 'digimon-enriched.json');
const CACHE_FILE = path.join(SCRAPER_DIR, '.cache', 'scrape-cache.json');
const IMAGES_DIR = path.join(SCRAPER_DIR, 'src', 'images');
...
export function canDeleteOutput(inputFile, outputFile) {
  return path.resolve(inputFile) !== path.resolve(outputFile) && path.basename(outputFile) === path.basename(OUTPUT_FILE);
}
